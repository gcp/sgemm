  ...
  for (int m0 = 0; m0 < pPrm->M; m0 += cc_nRows) {
    ...
    for (int m = m0; m < mLastEv; A += lda*2, m += 2) {
      for (int n = 0; n < ldcc; n += COLS_PER_LOOP) {
        ...
        pCc[3]      = _mm256_mul_ps(acc30, alpha_ps);
        pCc[4]      = _mm256_mul_ps(acc40, alpha_ps);
        pCc[ldcc+0] = _mm256_mul_ps(acc01, alpha_ps);
        pCc[ldcc+1] = _mm256_mul_ps(acc11, alpha_ps);
        pCc[ldcc+2] = _mm256_mul_ps(acc21, alpha_ps);
        pCc[ldcc+3] = _mm256_mul_ps(acc31, alpha_ps);
        pCc[ldcc+4] = _mm256_mul_ps(acc41, alpha_ps);
        pCc += COLS_PER_LOOP;
      }
      pCc += ldcc;
    }
...
void fma256_noncblas_sgemm(
 ...
 float *C, int ldc)
{
  fma256_noncblas_sgemm_multC(M, N, beta, C, ldc);
  ...
  for (int ri = 0; ri < n_Rsteps; ++ri) {
    ...
    for (int ci = 0; ci < n_Csteps; ++ci) {
      ...
      fma256_noncblas_sgemm_core(&prm, &A[row], &C[col]);
      col += bb_nCols;
    }
    if (col < N) {
      ...
      fma256_noncblas_sgemm_core_rightmostColumns(&prm, &A[row], &C[col], N-col, bb_nRows);
    }
    row += bb_nRows;
  }
  if (row < K) {
    ...
    for (int ci = 0; ci < n_Csteps; ++ci) {
      ...
      fma256_noncblas_sgemm_core_bottomRows(&prm, &A[row], &C[col], K-row);
      col += bb_nCols;
    }
    if (col < N) {
      ...
      fma256_noncblas_sgemm_core_rightmostColumns(&prm, &A[row], &C[col], N-col, K-row);
    }
  }
}
